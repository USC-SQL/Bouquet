...
public class ListAnalysis extends ForwardFlowAnalysis {
    ...
    public  HashMap<String,Set<NameValuepair>> parametertable=new HashMap<String,Set<NameValuepair>>();
    //private Set<Value>
    public ListAnalysis(UnitGraph graph) {
        super(graph);
        this.g = graph;
        ...
        for (Iterator unitIt = graph.iterator(); unitIt.hasNext(); ) {
            ...
            if("<org.apache.http.message.BasicNameValuePair: void <init>(java.lang.String,java.lang.String)>".equals(ToolKit.getInvokeSignature(stmt))
    ...
    protected void flowThrough(Object inValue, Object unit, Object outValue) {
        ...
        else if("<java.util.List: boolean add(java.lang.Object)>".equals(ToolKit.getInvokeSignature(stmt)))
        {
            Stmt s=(Stmt)stmt;
            InstanceInvokeExpr insinvoke=(InstanceInvokeExpr)s.getInvokeExpr();
            Value currentvalue=insinvoke.getBase();
            Value paravalue=insinvoke.getArg(0);
            ...
            //prepare the data for list
            Set<NameValuepair> setto;
            if(parametertable.containsKey(valuekey))
            {
                setto=parametertable.get(valuekey);
            }
            else{
                setto=new HashSet<NameValuepair>();
            ...
            for(Object obj:in)
            {
                ...
                if(currentvalue.toString().equals(killstringv))
                {
                    if(parametertable.containsKey(killp.key))
                    {
                        setto.addAll(parametertable.get(killp.key));
                    }
                }
                else if(paravalue.toString().equals(killstringv))
                {
                    NameValuepair nvp=new NameValuepair();
                    nvp.nameaddress=killp.value+"_1";
                    nvp.valueaddress=killp.value+"_2";
                    setto.add(nvp);

                }
            }
            parametertable.put(valuekey,setto);

        }
        else if("<org.apache.http.client.entity.UrlEncodedFormEntity: void <init>(java.util.List)>".equals(ToolKit.getInvokeSignature(stmt))
                ||"<org.apache.http.client.methods.HttpEntityEnclosingRequestBase: void setEntity(org.apache.http.HttpEntity)>".equals(ToolKit.getInvokeSignature(stmt)))
        {
            Stmt s=(Stmt)stmt;
            InstanceInvokeExpr insinvoke=(InstanceInvokeExpr)s.getInvokeExpr();
            ...
            Set<NameValuepair> setto;
            String valuekey=base.toString()+"@"+method.getSignature()+offset;
            if(parametertable.containsKey(valuekey))
            {
                setto=parametertable.get(valuekey);
            }
            ...
            for(Object obj:in)
            {
                ...
                if(parameter.toString().equals(tstringv))
                {
                    ...
                    genp.value=valuekey;
                    gen.add(genp);
                    if(parametertable.containsKey(tp.key))
                    {
                        setto.addAll(parametertable.get(tp.key));
                    }
                }
